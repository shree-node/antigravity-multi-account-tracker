...
class QuotaWidget(tk.Tk):
    def __init__(self):
        ...
        icon_label = tk.Label(self.title_bar, text="🚀", bg=title_bg, fg=title_fg, font=("Segoe UI Emoji", 15))
        ...
        try:
            ...
            if data:
                ...
                if not self.current_data:
                    self.after(0, self._show_error, "🔴 Could not fetch quota data.\nCheck your CLI & network.")
        except Exception:
            if not self.current_data:
                self.after(0, self._show_error, "🔴 Connection Error")
        finally:
            self.spinners["global"] = False
            self.is_fetching = False
    ...
    def _update_ui(self):
        ...
        for account in data_snapshot:
            ...
            tk.Label(header, text=f"👤 {email_short}", bg="#18181B", fg="#EA580C",
            ...
            for cat in ["Claude", "Gemini Pro", "Gemini Flash", "GPT"]:
